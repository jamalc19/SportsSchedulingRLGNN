...
def main():
    ...
    agent = RLAgent(CACHE_SIZE=CACHE_SIZE,EMBEDDING_SIZE=EMBEDDING_SIZE,EPS_START=EPS_START,EPS_END=EPS_END,EPS_STEP=EPS_STEP,
                    GAMMA=GAMMA,N_STEP_LOOKAHEAD=N_STEP_LOOKAHEAD, S2V_T = S2V_T, BATCH_SIZE=BATCH_SIZE, S2V=S2V, IMITATION_LEARNING_EPISODES=IMITATION_LEARNING_EPISODES)
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if RESTRICTED_ACTION_SPACE:
                q_value_dict = agent.Q(graph, slot=currentslot)
            else:
            ...
            if t >= TRAINING_DELAY:
                if t % OPTIMIZE_FREQUENCY:
                    agent.batch_train(e,RESTRICTED_ACTION_SPACE)
        ...
        if (t >= TRAINING_DELAY) and (e % ROLLOUT_FREQUENCY == 0):  # rollout
            ...
            for i in instances:
                ...
                if solutionlength==int(fullsolutionsize):
                    feasiblecount+=1
            print("%Feasible: {}".format(feasiblecount/len(instances)))
...
SAVE_FREQUENCY = 10
IMITATION_LEARNING_EPISODES=100

# Declare training instances
INSTANCE_SUMMARY = '4Teams'
RUN_NAME = 'S2VT=1FourTeamInstances100ImitationLearning'
